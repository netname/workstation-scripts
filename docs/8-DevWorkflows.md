> **Development Workstation** · [Overview](0-Overview.md) · [Stack](1-Stack.md) · [Installation](2-Installation.md) · [Terminal](3-Terminal.md) · [Projects](4-Projects.md) · [Editors](5-Editors.md) · [Desktop](6-Desktop.md) · [Troubleshooting](7-Troubleshooting.md) · **Workflows**
...
| **Part 9** | AI-assisted development: Claude Code | For interactive AI coding |
...
### Part 1: tmux — Daily Reference

> Full tmux documentation — model, layouts, keybindings, plugin configuration — is in [3-Terminal.md](3-Terminal.md) Parts 4–5.

**The two principles to internalize:**

- One WezTerm window is enough. All work happens inside tmux sessions.
- Sessions are recipes, not snapshots. Use `Ctrl-f` to recreate any session in under three seconds — this is why tmux-resurrect is excluded.

**Standard workspace layouts** (the sessionizer creates these from your project type):

| Layout | Windows | Use when |
|---|---|---|
| **A — Development** | editor (Neovim + shell pane), services (Docker/server logs), git (lazygit) | Any project with a running service |
| **B — Documentation** | write (Neovim, full width), git (lazygit) | Docs repos, no service component |
| **C — Exploration** | scratch (two shell panes) | Ad-hoc investigation, one-off scripts |

**Keybinding quick reference** (prefix = `Ctrl-Space`):

| Action | Binding |
|---|---|
| Open sessionizer picker | `Ctrl-f` |
| Detach (sessions keep running) | `prefix d` |
...
| Move focus (Neovim ↔ tmux) | `Ctrl-h/j/k/l` |
| Zoom pane (toggle fullscreen) | `prefix z` |
| Reload `tmux.conf` | `prefix r` |

> [!tip] **If `Ctrl-h/j/k/l` stop crossing the Neovim/tmux boundary:** the vim-tmux-navigator keybindings got out of sync. Check [5-Editors.md](5-Editors.md) §10.14 (Neovim side) and [3-Terminal.md](3-Terminal.md) §4.9 (tmux side). Reload both: `prefix r` for tmux, `:Lazy sync` for Neovim.

> [!important] **tmux is a user-managed dotfile.** Changes to keybindings go in `~/.config/tmux/tmux.conf` directly — not in Home Manager. Reload with `prefix r`. No Home Manager rebuild needed.

---

### Part 1B: Development Environment — Daily Reference

> Full devenv, direnv, and Docker Compose documentation is in [4-Projects.md](4-Projects.md). Home Manager is covered in [1-Stack.md](1-Stack.md) and [2-Installation.md](2-Installation.md).

**30-second environment check at session start:**

```bash
# Is the project environment active?
which python3        # /nix/store/... = active | /usr/bin/... = not loaded

# If not loaded:
direnv allow         # once-off approval for a new or changed .envrc
direnv reload        # force re-evaluation if already allowed

# Start services
docker compose up -d

# Confirm all services are up
docker compose ps    # all rows should show "Up"
```

**Adding a package to your project environment** (edit `devenv.nix`, then):

```bash
direnv reload        # or: cd .. && cd -
```

**Adding a global tool** (edit `~/dotfiles/home.nix`, then):

```bash
hms                              # apply the change (~30–60 s)
home-manager generations         # find the previous generation store path
# Then: /nix/store/<prev-generation-path>/activate   (undo if something breaks)
home-manager generations         # view generation history
```

> [!important] Commit every change to `~/dotfiles/` and push. An uncommitted change exists only on this machine. See [2-Installation.md](2-Installation.md) §2.6 for the verification checklist and configuration lifecycle.

---

### Part 1C: Neovim — Daily Reference

> Full Neovim and VS Code documentation — plugins, LSP setup, keybindings, configuration — is in [5-Editors.md](5-Editors.md).

**Open Neovim:**

```bash
nvim .              # open current directory as a project
nvim path/to/file   # open a specific file
```

**Leader key** = `Space`. Press Space and wait briefly for the which-key popup listing all commands.
...
**Navigating code (cursor on a symbol):**
...
| Shortcut | Action |
|---|---|
| `i` / `a` / `o` | Insert mode (before / after cursor / new line below) |
| `Escape` | Return to Normal mode |
| `u` / `Ctrl-r` | Undo / redo |
| `<leader>ca` | Code actions |
| `<leader>rn` | Rename symbol |
| `<leader>bd` | Close buffer (keep Neovim open) |
...
| Shortcut | Action |
|---|---|
| `<leader>cd` | Show full diagnostic on current line |
| `[d` / `]d` | Previous / next diagnostic |
| `<leader>xx` | All diagnostics in project (Trouble panel) |

**If the LSP stops working** (no gutter errors, `gd` says "No definition found"):

```bash
which pyright-langserver    # must return a /nix/store/... path
direnv allow                # or: direnv reload — if the env was not loaded
```

Then inside Neovim: `:LspRestart`

> [!note] **Using VS Code instead of Neovim?** Parts 2–8 (git, PR, and CI workflows) use `git`, `gh`, and `lazygit` from the terminal and are editor-agnostic. For file editing, use VS Code's built-in Source Control panel for staging and diff review. For VS Code setup specific to this stack, see [5-Editors.md](5-Editors.md) §11.
...
`git lg` is an alias for a formatted log (configured in [4-Projects.md](4-Projects.md) §9.2). It shows a compact, graph-decorated history.
...
`git st` is an alias for `git status --short` (configured in [4-Projects.md](4-Projects.md) §9.2). It shows staged and unstaged changes in a compact format.
...
gh will prompt for title, body, assignees, and labels one at a time. The body opens in your `$EDITOR` (Neovim, configured in [4-Projects.md](4-Projects.md) §9.2).
...
> [!important] `git co` is an alias for `git checkout -b` (configured in [4-Projects.md](4-Projects.md) §9.2). It creates the branch and switches to it in one step. If the branch already exists, it fails instead of resetting it. Use `git sw` or `git switch` when you want to switch to an existing branch.
...
This opens Neovim (configured as `$EDITOR` in [4-Projects.md](4-Projects.md) §9.2). Write the subject on the first line, leave a blank line, then write the body.
...
Pre-commit hooks run automatically every time you run `git commit`. They enforce code quality standards before the commit is recorded. Your project uses either `devenv.nix` git-hooks or `.pre-commit-config.yaml` — not both ([4-Projects.md](4-Projects.md) §8.8).
...
> [!important] **Pre-commit hooks must be installed before any AI-assisted coding session.** If `pre-commit install` was not run (or `just setup` was not run — which includes it), AI-generated commits bypass all quality hooks silently. Verify before starting:
>
> ```bash
...
**When to use this:** your feature branch has not been shared with anyone else (no teammate has pulled it), and you want to bring new `main` commits into your branch.
...
`git pushf` is an alias for `git push --force-with-lease --force-if-includes` (configured in [4-Projects.md](4-Projects.md) §9.2).
...
`gh pr diff` uses delta for highlighting (configured in [4-Projects.md](4-Projects.md) §9.2 — `gh config set pager delta`).
...
> [!tip] Neovim with the gitsigns plugin (configured in [5-Editors.md](5-Editors.md)) shows inline blame annotations as virtual text on the current line. For quick lookups while editing, this is faster than running `git blame` in the terminal. Use the terminal command for range queries and for piping into other tools.
...
### Part 9: AI-Assisted Development with Claude Code

Claude Code is the `claude` CLI — a conversational coding assistant with direct access to your files and codebase. It is interactive and immediate, handling tasks that require reading, searching, and modifying actual files.

---
---

#### 9.1 Claude Code Overview

Claude Code is the primary AI tool in this setup. Run it from any shell pane in your tmux workspace:

| Tool | Access | Best for |
|---|---|---|
| **Claude Code** (`claude`) | Any shell | Interactive editing, codebase navigation, targeted generation, debugging |

**Use Claude Code** when you want to have a back-and-forth conversation with an AI that can actually read your files, modify them, search the codebase, and run commands.

---

#### 9.2 Starting a Claude Code Session
...
#### 9.4 Claude Code and Your Git Workflow

Claude Code edits files but does not make commits on its own. After Claude Code modifies files, you review the changes and commit them yourself — the same workflow as any other code change:
...
#### 9.5 Giving Claude Code Better Context
...
All functions in this part belong in the `programs.zsh.initContent` block in `home.nix` ([3-Terminal.md](3-Terminal.md) §7.4). After adding or editing any function:
...
> [!warning] **Symptom:** `prs` shows `(gh not authenticated)`. **Cause:** `gh auth status` is failing — the GitHub CLI token has expired or `gh auth login` was never run. **Resolution:** `gh auth login` and follow the prompts. This is one of the three post-bootstrap manual steps ([2-Installation.md](2-Installation.md) §2.5).
...
# Scenario C: remote branch has new commits (shared branch)
git pull --rebase
```
...
#### 11.14 Tmux Keybindings (→ Part 1)

_Prefix key: `C-Space` (configured in `tmux.conf` — see [3-Terminal.md](3-Terminal.md) §4.9)_
...
#### 11.15 Shell Utility Functions (→ Part 10)
...
#### 11.16 Aliases Reference (→ [4-Projects.md](4-Projects.md) §9.2)
...
#### 11.18 Pre-flight Checklist — Before Starting Work
...
#### 11.19 Pre-flight Checklist — Before Opening a PR
...
This is one of the three post-bootstrap manual steps ([2-Installation.md](2-Installation.md) §2.5). After re-authenticating, verify:
...
Resolution: Update git. On Ubuntu with Home Manager, update the `pkgs.git` input in your flake ([2-Installation.md](2-Installation.md) §2.3).
...
If you manage git config through Home Manager ([4-Projects.md](4-Projects.md) §9.2), add it there and rebuild:
...
If the issue persists, check [5-Editors.md](5-Editors.md) §10.14 (Neovim side) and [3-Terminal.md](3-Terminal.md) §4.9 (tmux side) and confirm both sets of bindings are present and matching. Then reload Neovim plugins:
...
Cause: The session name does not match any directory in the sessionizer's search path. The sessionizer finds projects by scanning a defined set of directories (configured in the sessionizer script — [3-Terminal.md](3-Terminal.md) §5.5). A manually created session with `tmux new-session -s name` will not appear in the sessionizer picker if its name does not correspond to a project directory.
...
Cause 2: The direnv hook is not loaded early enough in the shell initialization sequence. The direnv hook must be loaded before tmux fires ([3-Terminal.md](3-Terminal.md) §7.4).
...
│  Keep branch current:                                               │
│    git fetch origin                                                 │
│    git rebase origin/main   (solo unshared branch)                 │
│    git merge origin/main    (shared branch)                         │
│    git pushf                                                        │
└──────────────────────────────┬──────────────────────────────────────┘
                               │
                               ▼
...
**Conventional Commits** A commit message format: `type(scope): description`. Types include `feat`, `fix`, `chore`, `docs`, `refactor`, `test`. Enables readable history and auto-generated changelogs.
...
**Sessionizer** A custom script that fuzzy-finds project directories and creates or attaches to named tmux sessions. Invoked with `Ctrl-f`. A user-managed dotfile — not managed by Home Manager. Covered in [3-Terminal.md](3-Terminal.md) §5.5.
